...
for scan_ind, state, target in data.scans():
    ...
    #geom NO LONGER NEEDED as fringe stopping works  -Tony
    ...
    vis *= np.exp(2j * np.pi * (cable_delay_turns))
...
for scan_ind, state, target in data.scans():
    ...
    # Visibility <A, B*> has phase (A - B), therefore add (B - A) phase to stop fringes (i.e. do delay tracking)
    #    vis *= np.exp(2j * np.pi * (geom_delay_turns + cable_delay_turns))
    vis *= np.exp(2j * np.pi * (cable_delay_turns))     #Tony
...
for scan_ind, state, target in data.scans():
    ...
    # Number of turns of phase that signal B is behind signal A due to geoetmric delay
